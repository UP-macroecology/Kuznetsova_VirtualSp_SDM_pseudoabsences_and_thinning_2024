...
# initial variables 'bio10 & 'bio14' have correlation of -0.50
...
## Fit RF 
m_rf_20_10 <- randomForest( x=sp_train[,my_preds], y=sp_train$occ, 
                      ntree=1000, importance =T)

# Variable importance:
importance(m_rf_20_10,type=1)
varImpPlot(m_rf_20_10)
...
xyz$z <- predict(m_rf_20_10, xyz, type='response')
...
par(mfrow=c(1,2)) 
partial_response(m_rf_20_10, predictors = sp_train[,my_preds], 
                 main='Random Forest', 
...
(perf_rf_20_10 <- evalSDM(sp_test$occ, predict(m_rf_20_10, 
                                         sp_test[,my_preds],  
                                         type='response') ))
...
## Fit RF 
m_rf_20_10_chb <- randomForest( x=sp_train[,my_preds], y=sp_train$occ, 
                            ntree=1000, importance =T)

# Variable importance:
importance(m_rf_20_10_chb,type=1)
varImpPlot(m_rf_20_10_chb)
# Interpretation: bio10 is crucial for the model. While bio14 might not drastically 
# impact the overall accuracy of the model when excluded, it still plays a 
# significant role in improving node purity and making effective splits in the decision trees.

# Look at single trees:
head(getTree(m_rf_20_10_chb,1,T))



# Make predictions to gradients:
xyz$z <- predict(m_rf_20_10_chb, xyz, type='response')
...
# Plot partial response curves:
par(mfrow=c(1,2)) 
partial_response(m_rf_20_10_chb, predictors = sp_train[,my_preds], 
                 main='Random Forest', 
                 ylab='Occurrence probability')
...
# Performance measures of RF
(perf_rf_20_10_chb <- evalSDM(sp_test$occ, predict(m_rf_20_10_chb, 
                                               sp_test[,my_preds],  
                                               type='response') ))
...
## Fit RF 
m_rf_20_5 <- randomForest( x=sp_train[,my_preds], y=sp_train$occ, 
                            ntree=1000, importance =T)

# Variable importance:
importance(m_rf_20_5,type=1)
varImpPlot(m_rf_20_5)
# Interpretation: bio10 is crucial for the model. While bio14 might not drastically 
# impact the overall accuracy of the model when excluded, it still plays a 
# significant role in improving node purity and making effective splits in the decision trees.
...
# Now, we plot the response surface:
xyz$z <- predict(m_rf_20_5, xyz, type='response')
...
# Plot partial response curves:
par(mfrow=c(1,2)) 
partial_response(m_rf_20_5, predictors = sp_train[,my_preds], 
                 main='Random Forest', 
                 ylab='Occurrence probability')
...
# Performance measures of RF
(perf_rf_20_5 <- evalSDM(sp_test$occ, predict(m_rf_20_5, 
                                               sp_test[,my_preds],  
                                               type='response') ))
...
# Inspect weakly correlated variables
var_sel$pred_sel
#Output: [1] "bio18" "bio2"  "bio1"  "bio3"  "bio15" "bio13" "bio6" 
...
## Fit RF 
m_rf_20_5_chb <- randomForest(x=sp_train[,my_preds], y=sp_train$occ, 
                                ntree=1000, importance =T)

# Variable importance:
importance(m_rf_20_5_chb,type=1)
varImpPlot(m_rf_20_5_chb)
# Interpretation: bio10 is crucial for the model. While bio14 might not drastically 
# impact the overall accuracy of the model when excluded, it still plays a 
...
# Look at single trees:
head(getTree(m_rf_20_5_chb,1,T))



# Make predictions to gradients:
xyz$z <- predict(m_rf_20_5_chb, xyz, type='response')
...
# Plot partial response curves:
par(mfrow=c(1,2)) 
partial_response(m_rf_20_5_chb, predictors = sp_train[,my_preds], 
                 main='Random Forest', 
                 ylab='Occurrence probability')
...
# Performance measures of RF
(perf_rf_20_5_chb <- evalSDM(sp_test$occ, predict(m_rf_20_5_chb, 
                                                   sp_test[,my_preds],  
                                                   type='response') ))
...
#Output: [1] "bio1"  "bio16" "bio19" "bio6"  "bio17" "bio8"  "bio15" "bio3"  "bio7"  "bio9" 
...
## Fit RF 
m_rf_20_3 <- randomForest( x=sp_train[,my_preds], y=sp_train$occ, 
                           ntree=1000, importance =T)

# Variable importance:
importance(m_rf_20_3,type=1)
varImpPlot(m_rf_20_3)
# Interpretation: bio10 is crucial for the model. While bio14 might not drastically 
# impact the overall accuracy of the model when excluded, it still plays a 
# significant role in improving node purity and making effective splits in the decision trees.
...
# Now, we plot the response surface:
xyz$z <- predict(m_rf_20_3, xyz, type='response')
...
# Plot partial response curves:
par(mfrow=c(1,2)) 
partial_response(m_rf_20_3, predictors = sp_train[,my_preds], 
                 main='Random Forest', 
                 ylab='Occurrence probability')
...
# Performance measures of RF
(perf_rf_20_3 <- evalSDM(sp_test$occ, predict(m_rf_20_3, 
                                              sp_test[,my_preds],  
                                              type='response') ))
...
# Inspect weakly correlated variables
var_sel$pred_sel
#Output: [1] "bio18" "bio4"  "bio8"  "bio3"  "bio1"  "bio15" "bio2"  "bio19" "bio16"
...
## Fit RF 
m_rf_20_3_chb <- randomForest(x=sp_train[,my_preds], y=sp_train$occ, 
                              ntree=1000, importance =T)

# Variable importance:
importance(m_rf_20_3_chb,type=1)
varImpPlot(m_rf_20_3_chb)
# Interpretation: bio10 is crucial for the model. While bio14 might not drastically 
# impact the overall accuracy of the model when excluded, it still plays a 
# significant role in improving node purity and making effective splits in the 
# decision trees.
...
# Look at single trees:
head(getTree(m_rf_20_3_chb,1,T))



# Make predictions to gradients:
xyz$z <- predict(m_rf_20_3_chb, xyz, type='response')
...
# Plot partial response curves:
par(mfrow=c(1,2)) 
partial_response(m_rf_20_3_chb, predictors = sp_train[,my_preds], 
                 main='Random Forest', 
                 ylab='Occurrence probability')
...
# Performance measures of RF
(perf_rf_20_3_chb <- evalSDM(sp_test$occ, predict(m_rf_20_3_chb, 
                                                  sp_test[,my_preds],  
                                                  type='response') ))
...
cor_mat 
# We are picking two variables representing temperature and precipitation
# initial variables 'bio10 & 'bio14' have correlation of -0.41
my_preds <- c('bio10','bio14')
...
## Fit RF 
m_rf_20_1 <- randomForest( x=sp_train[,my_preds], y=sp_train$occ, 
                           ntree=1000, importance =T)

# Variable importance:
importance(m_rf_20_1,type=1)
varImpPlot(m_rf_20_1)
# Interpretation: bio10 is crucial for the model. While bio14 might not drastically 
# impact the overall accuracy of the model when excluded, it still plays a 
# significant role in improving node purity and making effective splits in the decision trees.
...
# Now, we plot the response surface:
xyz$z <- predict(m_rf_20_1, xyz, type='response')
...
# Plot partial response curves:
par(mfrow=c(1,2)) 
partial_response(m_rf_20_1, predictors = sp_train[,my_preds], 
                 main='Random Forest', 
                 ylab='Occurrence probability')
...
# Performance measures of RF
(perf_rf_20_1 <- evalSDM(sp_test$occ, predict(m_rf_20_1, 
                                              sp_test[,my_preds],  
                                              type='response') ))
...
# Inspect weakly correlated variables
var_sel$pred_sel
#Output: [1] "bio12" "bio10" "bio3"  "bio7"  "bio18" "bio15" "bio11" "bio8" 

cor_mat 
# We are picking two variables representing temperature and precipitation
# initial variables 'bio10 & 'bio14' have correlation of -0.26
my_preds <- c('bio10','bio14')
...
## Fit RF 
m_rf_20_1_chb <- randomForest(x=sp_train[,my_preds], y=sp_train$occ, 
                              ntree=1000, importance =T)

# Variable importance:
importance(m_rf_20_1_chb,type=1)
varImpPlot(m_rf_20_1_chb)
# Interpretation: bio10 is crucial for the model. While bio14 might not drastically 
# impact the overall accuracy of the model when excluded, it still plays a 
# significant role in improving node purity and making effective splits in the 
# decision trees.
...
# Make predictions to gradients:
xyz$z <- predict(m_rf_20_1_chb, xyz, type='response')
...
# Plot partial response curves:
par(mfrow=c(1,2)) 
partial_response(m_rf_20_1_chb, predictors = sp_train[,my_preds], 
                 main='Random Forest', 
                 ylab='Occurrence probability')
...
# Performance measures of RF
(perf_rf_20_1_chb <- evalSDM(sp_test$occ, predict(m_rf_20_1_chb, 
                                                  sp_test[,my_preds],  
                                                  type='response') ))
...
(comp_perf <- rbind(rf_20x10 = perf_rf_20_10, rf_20x10_chb = perf_rf_20_10_chb,
                    rf_20x5 = perf_rf_20_5, rf_20x5_chb = perf_rf_20_5_chb,
                    rf_20x3 = perf_rf_20_3, rf_20x3_chb = perf_rf_20_3_chb,
                    rf_20x1 = perf_rf_20_1, rf_20x1_chb = perf_rf_20_1_chb))
...
write.table(comp_perf, file='data/RF_perf_20x10x5x3x1_spThin_chb.txt', row.names=F)